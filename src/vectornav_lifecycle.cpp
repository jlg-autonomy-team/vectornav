...
{
  if (get_current_state().id() != lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {
    RCLCPP_ERROR(get_logger(), "Device must be active.");
    resp->success = false;
    return;
  }

  if (req->duration <= 0) {
    RCLCPP_ERROR(get_logger(), "Duration must be greater than 0");
    resp->success = false;
    return;
  }
  ...
  get_clock()->sleep_for(std::chrono::microseconds(static_cast<uint64_t>(req->duration*1e6)));
