...
VectornavLifecycleNode::VectornavLifecycleNode()
: LifecycleNode("vectornav")
, vs_{}
{
  RCLCPP_INFO(get_logger(), "Initializing vectornav device");
  main_loop_timer_ = create_wall_timer(
    std::chrono::milliseconds(50),
    std::bind(&VectornavLifecycleNode::main_loop, this));
...
VectornavLifecycleNode::~VectornavLifecycleNode()
{
  ...
  if (get_current_state().id() == lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {
    RCLCPP_DEBUG(get_logger(), "Unconfiguring device");
    unconfigure_device();

    std::this_thread::sleep_for(std::chrono::milliseconds(200));
    RCLCPP_DEBUG(get_logger(), "Disconnecting device");
    disconnect_device();
...
void VectornavLifecycleNode::read_parameters()
{
...
void VectornavLifecycleNode::advertise_topics()
{
...
void VectornavLifecycleNode::delete_topics()
{
  pub_imu_.reset();
  pub_mag_.reset();
  pub_temp_.reset();
...
void VectornavLifecycleNode::advertise_services()
{
  if (params_.acc_bias_enable) {
    srv_reset_acc_bias_ = create_service<std_srvs::srv::Trigger>(
      "~/reset_acc_bias", std::bind(&VectornavLifecycleNode::reset_acc_bias, this, std::placeholders::_1, std::placeholders::_2));
  ...
  if (device_family_ != vn::sensors::VnSensor::Family::VnSensor_Family_Vn100) {
    srv_reset_odom_ = create_service<std_srvs::srv::Empty>(
      "reset_odom", std::bind(&VectornavLifecycleNode::reset_odometry, this, std::placeholders::_1, std::placeholders::_2));
  }
}

void VectornavLifecycleNode::delete_services()
{
  if (params_.acc_bias_enable) {
    srv_reset_acc_bias_.reset();
    srv_set_acc_bias_.reset();
  }
  srv_reset_odom_.reset();
}

void VectornavLifecycleNode::reset_acc_bias(
  const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
  std::shared_ptr<std_srvs::srv::Trigger::Response> resp)
{
  std::unique_lock<std::mutex> service_lock(service_acc_bias_mtx_);
  RCLCPP_INFO(get_logger(), "Reset to factory new acceleration bias.");
...
void VectornavLifecycleNode::set_acc_bias(
    const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
    std::shared_ptr<std_srvs::srv::Trigger::Response> resp)
{
  ...
  std::unique_lock<std::mutex> lock(mtx_samples_);
    samples_.clear();
    samples_.reserve(static_cast<size_t>(params_.set_acc_bias_seconds * params_.fixed_imu_rate * 1.5));
    take_samples_ = true;
    auto const start = now();
  lock.unlock();

  auto const time {rclcpp::Duration{std::chrono::microseconds{static_cast<uint64_t>(params_.set_acc_bias_seconds*1e6)}}};
  get_clock()->sleep_for(time);
  ...
  if (samples_.size() < 10) {
    RCLCPP_ERROR(get_logger(), "Not enough samples taken (<10). Aborting.");
    resp->message = "Not enough samples taken (<10). Aborting.";
    resp->success = false;
  } else {
    RCLCPP_INFO(get_logger(), "Applying bias correction to vectornav:");
    RCLCPP_INFO(get_logger(), " - Samples taked: %lu (%.2lfs)", samples_.size(), (end - start).seconds());
    RCLCPP_INFO(get_logger(), " - Bias:       [x: %7.4lf, y: %7.4lf, z: %7.4lf]", bias.x, bias.y, bias.z);
    RCLCPP_INFO(get_logger(), " - Covariance: [x: %7.4lf, y: %7.4lf, z: %7.4lf]", covariance_x, covariance_y, covariance_z);
...
void VectornavLifecycleNode::connect_device(std::string const & port, int baudrate)
{
...
void VectornavLifecycleNode::disconnect_device()
{
  vs_->disconnect();
  std::this_thread::sleep_for(std::chrono::milliseconds(100));
}

void VectornavLifecycleNode::configure_device()
{
  int package_rate = 0;
  ...
  if (params_.has_rotation_reference_frame == true) {
  ...
  vs_->registerAsyncPacketReceivedHandler(static_cast<void*>(this), &VectornavLifecycleNode::binary_async_message_received);
}

void VectornavLifecycleNode::unconfigure_device()
{
  using namespace vn::protocol::uart;
  vn::sensors::BinaryOutputRegister bor_none(
    0, 1, COMMONGROUP_NONE, TIMEGROUP_NONE, IMUGROUP_NONE, GPSGROUP_NONE, ATTITUDEGROUP_NONE,
  ...
  vs_->writeBinaryOutput1(bor_none);
  vs_->writeBinaryOutput2(bor_none);
  vs_->writeBinaryOutput3(bor_none);
  
  // Unregister async callback function
  vs_->unregisterAsyncPacketReceivedHandler();
}

void VectornavLifecycleNode::binary_async_message_received(void* aux, vn::protocol::uart::Packet & p, size_t index)
{
  static_cast<VectornavLifecycleNode*>(aux)->binary_async_message_received_(p, index);
}

void VectornavLifecycleNode::binary_async_message_received_(vn::protocol::uart::Packet & p, size_t index)
{  
...
rclcpp::Time VectornavLifecycleNode::get_timestamp(vn::sensors::CompositeData & cd, const rclcpp::Time & ros_time)
{
...
bool VectornavLifecycleNode::validate_timestamp(double const sensor_time)
{
  bool isValid = true;
...
bool VectornavLifecycleNode::fill_imu_msg(vn::sensors::CompositeData & cd, sensor_msgs::msg::Imu & imu_msg, const rclcpp::Time & ros_time)
{
...
bool VectornavLifecycleNode::fill_mag_msg(vn::sensors::CompositeData & cd, sensor_msgs::msg::MagneticField & mag_msg, const rclcpp::Time & ros_time)
{
...
bool VectornavLifecycleNode::fill_temp_msg(vn::sensors::CompositeData & cd, sensor_msgs::msg::Temperature & temp_msg, const rclcpp::Time & ros_time)
{
...
bool VectornavLifecycleNode::fill_pres_msg(vn::sensors::CompositeData & cd, sensor_msgs::msg::FluidPressure & pres_msg, const rclcpp::Time & ros_time)
{
...
bool VectornavLifecycleNode::fill_gps_msg(vn::sensors::CompositeData & cd, sensor_msgs::msg::NavSatFix & gps_msg, const rclcpp::Time & ros_time)
{
...
bool VectornavLifecycleNode::fill_odom_msg(vn::sensors::CompositeData & cd, nav_msgs::msg::Odometry & odom_msg, const rclcpp::Time & ros_time)
{
  // Check with vectornav different of VN-100
...
bool VectornavLifecycleNode::fill_ins_msg(vn::sensors::CompositeData & cd, vectornav::msg::Ins & ins_msg, const rclcpp::Time & ros_time)
{
  // Check with vectornav different of VN-100
...
VectornavLifecycleNode::cb_return
VectornavLifecycleNode::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  return VectornavLifecycleNode::cb_return::SUCCESS;
}

VectornavLifecycleNode::cb_return
VectornavLifecycleNode::on_activate(const rclcpp_lifecycle::State & state)
{
  RCLCPP_INFO(get_logger(), "on_activate");
  LifecycleNode::on_activate(state);
  
  last_sensor_time_ = now().seconds();
...
VectornavLifecycleNode::cb_return
VectornavLifecycleNode::on_deactivate(const rclcpp_lifecycle::State & state)
{
  RCLCPP_DEBUG(get_logger(), "Unconfiguring device");
  unconfigure_device();
...
VectornavLifecycleNode::cb_return
VectornavLifecycleNode::on_cleanup(const rclcpp_lifecycle::State & state)
{
...
VectornavLifecycleNode::cb_return
VectornavLifecycleNode::on_shutdown(const rclcpp_lifecycle::State & state)
{
...
VectornavLifecycleNode::cb_return
VectornavLifecycleNode::on_error(const rclcpp_lifecycle::State & state)
{
  RCLCPP_INFO(get_logger(), "on_error");
  return VectornavLifecycleNode::cb_return::SUCCESS;
}

void VectornavLifecycleNode::main_loop()
{
